...
password.addEventListener("input", () => {
  passError.textContent = validatePassword(password.value) ? "" : "Пароль должен быть 8-20 символов, содержать заглавную, строчную букву, цифру и спецсимвол.";
});

confirmPassword.addEventListener("input", () => {
  confirmError.textContent = password.value === confirmPassword.value ? "" : "Пароли не совпадают!";
});
...
form.addEventListener("input", () => {
  submitBtn.disabled = !form.checkValidity() || passError.textContent || confirmError.textContent || birthError.textContent;
});
