var i18ObjLocal = {
    'ru': {
        ...
        'no-items': 'Ничего не найдено',
        'load-error': 'Ошибка загрузки каталога',
        'details-btn': 'Подробнее',
        'cart-btn': '🛒 В корзину',
        'fav-btn': '☆ В избранное',
        'auth-required': 'Нужна авторизация',
        'cart-added': 'Добавлено в корзину',
        'fav-added': 'Добавлено в избранное',
        'fav-exists': 'Уже в избранном',
        'price-label': 'Цена',
        ...
        'login-register-link': 'Зарегистрируйтесь',
        'login-error-server': 'Ошибка сервера. Проверьте, запущен ли json-server',
        'login-error-not-found': 'Пользователь с таким email не найден!',
        'login-error-password': 'Неверный пароль!',
        'login-error-connection': 'Ошибка подключения к серверу. Проверьте консоль.',
        'login-success': 'Успешный вход!',
        'login-already-logged': 'Вы уже авторизованы. Перейти в каталог?',
        'loading': 'Загрузка…',
    },
    'en': {
        // Navigation
...
window.getTranslate = function(newLang) {
    ...
    // Переводим элементы с data-i18n-key (для динамических кнопок)
    ...
    // Вызываем перерендер каталога, если функция существует
...
window.getLocalStorage = function() {
    ...
    if (savedTheme) {
        ...
        window.updateThemeSymbol(isDark ? '🌙' : '☀️');
...
// Переключение изображений для темы
...
if (mobileThemeBtn) mobileThemeBtn.addEventListener('click', window.toggleTheme);
