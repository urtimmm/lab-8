...
function checkAuth() {
    ...
    if (!userData) {
        favDiv.innerHTML = "<p>⚠️ Для просмотра избранного необходимо <a href='login.html'>авторизоваться</a></p>";
        return false;
    }
...
async function loadFavorites() {
    ...
    try {
        ...
        favDiv.innerHTML = data.length ? data.map(p => `
      <div class="card">
...
async function addToCart(productId) {
    ...
    const checkRes = await fetch(`${cartAPI}?userId=${currentUser.id}&productId=${productId}`);
    ...
    const favRes = await fetch(`${favAPI}?userId=${currentUser.id}&productId=${productId}`);
